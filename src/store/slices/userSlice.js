...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      "http://192.168.1.2:8000/api/v1/user/login",
      { email, password },
...
export const getUser = () => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get(
      "http://192.168.1.19:8000/api/v1/user/me",
      {
...
export const logout = () => async (dispatch) => {
  try {
    const { data } = await axios.get(
      "http://192.168.1.19:8000/api/v1/user/logout",
      { withCredentials: true }
    );
  ...
  (currentPassword, newPassword, confirmNewPassword) => async (dispatch) => {
    ...
    try {
      const { data } = await axios.put(
        "http://192.168.1.19:8000/api/v1/user/password/update",
...
export const updateProfile = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      "http://192.168.1.19:8000/api/v1/user/me/profile/update",
      data,
      {
        withCredentials: true,
